...
def main(args):
    ...
    if args.extract_movi_f:
        Parallel(n_jobs=args.num_workers)(
            delayed(extract_movi_f)(args.data_root, args.download_path, indices)
            for indices in worker_video_indices
        )
    if args.save_tracks:
        Parallel(n_jobs=args.num_workers)(
            delayed(save_tracks)(args.data_root, indices)
            for indices in worker_video_indices
        )
...
def save_tracks(data_root, video_indices):
    ...
    for video_idx in tqdm(video_indices):
        ...
        with torch.no_grad():
            tracks = model(
                {"video": video}, mode="tracks_at_motion_boundaries", **vars(args)
            )["tracks"]
