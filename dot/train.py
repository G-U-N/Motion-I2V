...
def sample(pred, gt):
    ...
    sparse = torch.nn.functional.grid_sample(
...
def step(loader, model, optimizer, logger, global_iter, args):
    ...
    motion_loss = torch.tensor(0.0, requires_grad=True).cuda()
    visibility_loss = torch.tensor(0.0, requires_grad=True).cuda()
    gamma = 0.8
    num_iter = pred["out_tgt_points"].size(1)
    for i in range(num_iter):
        weight = gamma ** (num_iter - i - 1)
        pred_pos, pred_vis = (
            pred["out_tgt_points"][:, i][..., :2],
            pred["out_tgt_points"][:, i][..., 2],
        )
        gt_pos, gt_vis = gt["out_tgt_points"][..., :2], gt["out_tgt_points"][..., 2]
        motion_loss += weight * (gt_pos - pred_pos).abs().mean()
        visibility_loss += weight * torch.nn.functional.binary_cross_entropy(
            pred_vis, gt_vis
        )
...
def main(rank, args):
    ...
    train_dataset = create_point_tracking_dataset(
        args, split="train", verbose=args.rank == 0
    )
    valid_dataset = create_point_tracking_dataset(
        args, split="valid", verbose=args.rank == 0, num_workers=1
    )
        ...
        if args.valid_iter > 0 and global_iter % args.valid_iter == 0:
            ...
            with torch.no_grad():
                for _ in range(args.num_valid_batches):
                    losses.append(
                        step(valid_dataset, model, None, None, global_iter, args)
                    )
            ...
            if args.rank == 0:
                ...
                print(
                    f"[I{global_iter}/{args.train_iter}] Validation loss: {loss:.3E} {status}"
                )
                model.train()
