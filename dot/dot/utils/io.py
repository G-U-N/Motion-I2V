...
def read_video(
    ...
    if path.endswith(".mp4"):
        ...
        video = read_video_from_folder(
            path, start_step, time_steps, channels, resolution, exts
        )
    return video
...
def write_video(
    video, path, channels="first", zero_padded=True, ext="png", dtype="torch"
):
    if dtype == "numpy":
...
def write_video_to_file(video, path, channels):
    ...
    torchvision.io.write_video(
        path, video, 24, "h264", options={"pix_fmt": "yuv420p", "crf": "23"}
    )
    return video
