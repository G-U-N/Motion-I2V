        ...
        if tracks is not None:
            ...
            if is_video:
                ...
                for t in range(time_steps):
                    ...
                    occ_t = occ[:, t]
                    new_tensor.append(
                        plot_tracks(tensor[:, t], pos_t, occ_t, tracks=traj[:, : t + 1])
                    )
...
def plot_tracks(rgb, points, occluded, tracks=None, trackgroup=None):
    ...
    points = points.transpose(
        1, 0, 2
    ).copy()  # clone, otherwise it updates points array
    # points[..., 0] *= (width - 1)
    # points[..., 1] *= (height - 1)
    if tracks is not None:
        tracks = tracks.copy()
    ...
    return (
        np.stack(disp, axis=0).astype(float).transpose(0, 3, 1, 2) / 255
    )  # TODO : inconsistent


def plot_points(
    src_frame, tgt_frame, src_points, tgt_points, save_path, max_points=256
):
    ...
    P = 10
    plt.imshow(
        np.concatenate((src_frame, np.ones_like(src_frame[:, :P]), tgt_frame), axis=1)
    )
    indices = np.random.choice(
        len(src_pos), size=min(max_points, len(src_pos)), replace=False
    )
    for i in indices:
        if src_alpha[i] == 1:
            ...
            ax.scatter(
                src_pos[i, 0], src_pos[i, 1], s=5, linewidths=1.5, c="black", marker="o"
            )
        ...
        if tgt_alpha[i] == 1:
            ...
            ax.scatter(
                W + P + tgt_pos[i, 0], tgt_pos[i, 1], s=2.5, c="white", marker="o"
            )

        plt.plot(
            [src_pos[i, 0], W + P + tgt_pos[i, 0]],
            [src_pos[i, 1], tgt_pos[i, 1]],
            linewidth=0.5,
            c="black",
        )
