...
class DenseOpticalTracker(nn.Module):
    def __init__(
        ...
        self.optical_flow_refiner = OpticalFlow(
            height, width, refiner_config, refiner_path
        )
    ...
    def get_flow_from_last_to_first_frame(self, data, **kwargs):
        ...
        init = self.point_tracker(data, mode="tracks_at_motion_boundaries", **kwargs)[
            "tracks"
        ]
        init = torch.stack(
            [init[..., 0] / (w - 1), init[..., 1] / (h - 1), init[..., 2]], dim=-1
        )
        data = {
    ...
    def get_tracks_for_queries(self, data, **kwargs):
        ...
        feats = self.optical_flow_refiner({"video": video}, mode="feats", **kwargs)[
            "feats"
        ]
        ...
        for src_step in range(src_steps):
            src_points = init[:, src_step]
            src_feats = feats[:, src_step]
            tracks_from_src = []
            for tgt_step in tqdm(
                range(time_steps), desc="Refine target step", leave=False
            ):
                if src_step == tgt_step:
                    ...
                    pred = self.optical_flow_refiner(
                        data, mode="flow_with_tracks_init", **kwargs
                    )
                    flow, alpha = pred["flow"], pred["alpha"]
                    flow[..., 0] = flow[..., 0] / (W - 1)
                    flow[..., 1] = flow[..., 1] / (H - 1)
                tracks_from_src.append(
                    torch.cat([flow + grid, alpha[..., None]], dim=-1)
                )
            ...
            cur_tracks = F.grid_sample(
                tracks_from_src, cur_grid, align_corners=True, mode="bilinear"
            )
    ...
    def get_tracks_from_first_to_every_other_frame(self, data, **kwargs):
        ...
        source_frame = (
            video[:, 0:1, ...].repeat(1, T - 1, 1, 1, 1).reshape(B * (T - 1), 3, H, W)
        )
        target_frame = video[:, 1:, ...].reshape(B * (T - 1), 3, H, W)

        _, _, NN, _ = init.shape

        source_point = (
            init[:, 0:1, ...].repeat(1, T - 1, 1, 1).reshape(B * (T - 1), NN, 3)
        )
        target_point = init[:, 1:, ...].reshape(B * (T - 1), NN, 3)

        data = {
            "src_frame": source_frame,
            "tgt_frame": target_frame,
            "src_points": source_point,
            "tgt_points": target_point,
        }
        pred = self.optical_flow_refiner(data, mode="flow_with_tracks_init", **kwargs)
        flow, alpha = pred["flow"], pred["alpha"]

        flow0 = torch.zeros(B, 1, H, W, 2, device=video.device)
        alpha0 = torch.ones(B, 1, H, W, 1, device=video.device)

        flow = torch.cat([flow0, flow.reshape(B, T - 1, H, W, 2)], dim=1)
        alpha = torch.cat([alpha0, alpha[None].reshape(B, T - 1, H, W, 1)], dim=1)

        tracks = torch.cat([flow, alpha], dim=-1)
        return {"tracks": tracks}
