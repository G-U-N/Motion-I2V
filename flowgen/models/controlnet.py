...
class ControlNetConditioningEmbedding(nn.Module):
    ...
    def __init__(
        ...
        #     nn.Conv3d(conditioning_channels, 64, kernel_size=3, padding=1),
        #     nn.SiLU(),
        #     nn.Conv3d(64, 128, kernel_size=3, padding=1),
        #     nn.SiLU(),
        #     nn.Conv3d(128, 256, kernel_size=1, padding=0),
        #     nn.SiLU(),
        #     nn.Conv3d(256, 512, kernel_size=1, padding=0),
        #     nn.SiLU(),
        #     zero_module(nn.Conv3d(512, conditioning_embedding_channels, kernel_size=1, padding=0))
        # )
        self.layers = nn.Sequential(
            nn.Conv3d(
                conditioning_channels, 512, kernel_size=(1, 3, 3), padding=(0, 1, 1)
            ),
            nn.SiLU(),
            zero_module(
                nn.Conv3d(
    ...
    def forward(self, conditioning):
        embedding = self.layers(conditioning)
        return embedding
...
class ControlNetModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.stride_embedding = TimestepEmbedding(timestep_input_dim, time_embed_dim)
    ...
    def from_unet(
        ...
        controlnet = cls(
            ...
            use_inflated_groupnorm=unet.config.use_inflated_groupnorm,
            # Additional
            use_motion_module=unet.config.use_motion_module,
            motion_module_resolutions=unet.config.motion_module_resolutions,
            motion_module_mid_block=unet.config.motion_module_mid_block,
            motion_module_decoder_only=unet.config.motion_module_decoder_only,
            motion_module_type=unet.config.motion_module_type,
            motion_module_kwargs=unet.config.motion_module_kwargs,
            unet_use_cross_frame_attention=unet.config.unet_use_cross_frame_attention,
            unet_use_temporal_attention=unet.config.unet_use_temporal_attention,
        )
        ...
        if load_weights_from_unet:
            ...
            controlnet.stride_embedding.load_state_dict(
                unet.stride_embedding.state_dict()
            )
    ...
    def forward(
        ...
        stride: Union[torch.Tensor, float],
        ...
        stride = stride.repeat(sample.shape[0] // stride.shape[0], 1)

        _b, _f = stride.shape
        stride = stride.reshape(_b * _f)

        t_emb = self.time_proj(timesteps)
        stride_emd = self.time_proj(stride)

        # timesteps does not contain any weights and will always return f32 tensors
        # but time_embedding might actually be running in fp16. so we need to cast here.
        # there might be better ways to encapsulate this.
        t_emb = t_emb.to(dtype=self.dtype)
        stride_emd = stride_emd.to(dtype=self.dtype)

        t_emb = t_emb.unsqueeze(1)
        stride_emd = stride_emd.reshape(_b, _f, -1)

        emb = self.time_embedding(t_emb) + self.stride_embedding(stride_emd)
        emb = emb.reshape(_b, _f, -1)
