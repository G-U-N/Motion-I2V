...
def prepare_imgs(folder, sample_size=(512, 512)):
    img_paths = sorted(os.listdir(folder))
    imgs = [
        torch.from_numpy(np.array(Image.open(os.path.join(folder, img_path))) / 255.0)
        .permute(2, 0, 1)
        .contiguous()
        .float()
    ...
    pixel_transforms_norm = transforms.Compose(
        [
    ...
    imgs_norm = pixel_transforms_norm(imgs.clone())
    imgs_unnorm = pixel_transforms(imgs.clone())

    return imgs_unnorm, imgs_norm
...
def main(args):
    ...
    video, _ = prepare_imgs("drift-turn", sample_size=(512, 512))
    video = video.cuda()
    ...
    this_sample = []
    this_sample.append(video[0:1].float().cpu())

    vis_mask = tracks.permute(0, 3, 1, 2)[:, 2:, ...].repeat(1, 3, 1, 1)
    tracks = tracks.permute(0, 3, 1, 2)[:, :2, ...]

    grids = coords_grid(1, 512, 512, "cuda")

    for idx in range(1, 16):
        # warped_img, o = forwardwarpFunc(first_frame_unnorm[None].float().cuda(), sample[:, :, i, ...].float().cuda())
        warped_img = softsplat(
            tenIn=video[0:1],
            tenFlow=tracks[idx : idx + 1] - grids,
            tenMetric=None,
            strMode="avg",
        )
    ...
    imgs = []
    for _idx in range(this_sample.shape[2]):
        imgs.append(
            (this_sample[0, :, _idx, ...].permute(1, 2, 0).numpy() * 255).astype(
                np.uint8
            )
        )
    img = np.hstack(imgs)
    masks = np.hstack(vis_mask.permute(0, 2, 3, 1).cpu().numpy() * 255)
    img = np.concatenate((img.astype(np.uint8), masks.astype(np.uint8)), axis=0)
    img = Image.fromarray(img)
    img.save(f"drift-turn.png")
