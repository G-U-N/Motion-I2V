...
class VersatileAttention(CrossAttention):
    def __init__(
        ...
        **kwargs,
        ...
        self.forwardwarpFunc = ForwardWarp()
    ...
    def forward_warp_hidden(self, flow, hidden_states):
        ...
        batch, video_length, _, H, _ = flow.shape  # [1, 6, 2, 40, 40]
        flow = (
            flow[:, 1:, ...]
            .reshape(batch * (video_length - 1), 2, H, H)
            .contiguous()
            .float()
        )  # [5, 2, 40, 40]
        hidden_states = hidden_states.reshape(
            batch, video_length, H, H, C
        )  # [1, 6, 40, 40, 320]
        hidden_states = hidden_states[:, 0, ...]
        hidden_states = hidden_states.permute(0, 3, 1, 2).contiguous().float()

        warped_img, o = self.forwardwarpFunc(
            hidden_states.repeat(video_length - 1, 1, 1, 1), flow
        )
        warped_img = warped_img / (o + 1e-6)  # [5, 320, 40, 40]

        return warped_img

        # flow = flow[:, 1:, ...]
        # _, _, C = hidden_states.shape
        # hidden_states = hidden_states.reshape(batch, video_length, H, H, C) # [1, 6, 40, 40, 320]
        # hidden_states = hidden_states[:, 0, ...]
        # hidden_states = hidden_states.detach().permute(0,3,1,2).contiguous().float()

        # _tmp = []

        # for idx in range(flow.shape[1]):
        #     warped_img, o = self.forwardwarpFunc(hidden_states, flow[:, idx, ...].contiguous().float())
        #     warped_img = warped_img / (o+1e-6) # [5, 320, 40, 40]
        #     _tmp.append(warped_img)

        # warped_img = torch.stack(_tmp, dim=1)

        # warped_img = warped_img.reshape(batch, video_length-1, C, H, H)

        # return warped_img.detach()
    ...
    def forward(
        ...
        if self.attention_mode == "Temporal":
            ...
            flow_pre = flow_pre.reshape(B_flow, L_flow, 2, H, H)  # [1, 6, 2, 40, 40]

            warped_hidden_states = self.forward_warp_hidden(
                flow_pre, hidden_states
            )  # [1, 5, 320, 40, 40]
            warped_hidden_states = warped_hidden_states.reshape(
                B_flow, L_flow - 1, C, H * H
            ).permute(0, 1, 3, 2)

            _tmp = []
            # print(hidden_states.shape, "!!!!!!!!!!!") # [6, 1600, 320]
            hidden_states = hidden_states.reshape(B_flow, L_flow, H * H, C)
            # warped_hidden_states = hidden_states[:, 1:, ...]
            # print(hidden_states.shape, "@@@@@@@@@@@") # [1, 6, 1600, 320]
            # print(warped_hidden_states.shape, "#############") # [1, 5, 1600, 320]
            ...
            hidden_states = hidden_states.reshape(-1, H * H, C)
