...
def get_correspondences(
    track_path, src_step, tgt_step, num_tracks, height, width, vis_src_only
):
    H, W = height, width
    ...
    if vis_src_only:
        ...
        idx = np.random.choice(
            tracks.size(0), num_tracks, replace=num_tracks > tracks.size(0)
        )
...
class Dataset(data.Dataset):
    def __init__(self, args, split="train", verbose=False):
        ...
        self.video_steps = [
            len(glob(os.path.join(self.video_folder, str(video_idx), "*")))
    ...
    def __getitem__(self, idx):
        ...
        vis_src_only = False
        _, corr = get_correspondences(
            in_track_path, src_step, tgt_step, self.num_in_tracks, H, W, vis_src_only
        )
        src_points, tgt_points = corr

        vis_src_only = True
        success, corr = get_correspondences(
            out_track_path, src_step, tgt_step, self.num_out_tracks, H, W, vis_src_only
        )
        if not success:
            return self[idx + 1]
