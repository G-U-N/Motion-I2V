...
def main(args):
    ...
    for index, gt in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        if len(args.plot_indices) > 0:
            ...
            plot_points(
                ...
                src_points,
                tgt_points,
                osp.join(args.result_path, f"{index}/points.png"),
            )
            write_video(
                gt["video"][0], osp.join(args.result_path, f"{index}/video.mp4")
            )
            write_frame(
                to_rgb(pred["flow"], "flow")[0],
                osp.join(args.result_path, f"{index}/pred_flow.png"),
            )
            write_frame(
                to_rgb(pred["coarse_flow"], "flow")[0],
                osp.join(args.result_path, f"{index}/coarse_flow.png"),
            )
            write_frame(
                to_rgb(gt["flow"], "flow")[0],
                osp.join(args.result_path, f"{index}/gt_flow.png"),
            )
            write_frame(
                to_rgb(pred["alpha"], "mask")[0],
                osp.join(args.result_path, f"{index}/pred_alpha.png"),
            )
            write_frame(
                to_rgb(gt["alpha"], "mask")[0],
                osp.join(args.result_path, f"{index}/gt_alpha.png"),
            )
            write_frame(
                to_rgb(pred["coarse_alpha"], "mask")[0],
                osp.join(args.result_path, f"{index}/coarse_alpha.png"),
            )
    ...
    if len(args.plot_indices) == 0:
        ...
        save_path = osp.join(
            args.log_path,
            f"cvo_metrics_{args.split}_{args.model}_{args.worker_idx}.txt",
        )
